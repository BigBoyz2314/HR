    ...
    if ($result->num_rows > 0) {
        ...
        while($row = $result->fetch_assoc()) { 
            ...
            if ($result5->num_rows == 0) {
                ...
                $remaining = $pay - $remain;

                echo $payable ."<br>";



                $sql3 ="UPDATE `salary` SET `basic_salary` = '$basic', `allowance` = '$allowance', `deduction` = '$deduction', `gross_salary` = '$gross', `total_days` = '$totaldays', `pay_days` = '$dayspayable', `absent` = '$absent', `payable` = '$pay', `paid` = '$pai', `remaining` = '$remaining', `updated_at` = current_timestamp())";
